...
<script>
	...
	export default {
		...
		methods: {
			...
			loadProvinces() { // 加载省份
			this.commHttpRequest('query_user', 'GetListProvince', {}, false, (res) => {
				let {data} = res.data
				this.districtsObj.provinces = data
				this.loadCities(data[0].code)
			})
			},
			loadCities(AreaId){
				this.commHttpRequest('query_user', 'GetListCity', {parent_code:AreaId}, false, (res) => {
					let {data} = res.data
					this.districtsObj.cities = data
					this.loadAreas(data[0].code)
				})
			},
			loadAreas(AreaId){
				this.commHttpRequest('query_user', 'GetListCity', {parent_code:AreaId}, false, (res) => {
			...
			getResult(event = null) {
				...
				//过滤北京、上海、重庆、天津 的省
				if(this.districtsObj.provinces[this.province].name=='北京市'||this.districtsObj.provinces[this.province].name=='上海市'||this.districtsObj.provinces[this.province].name=='重庆市'||this.districtsObj.provinces[this.province].name=='天津市'){
					result = {
						province: {"code":"","name":"","parent_code":""},
						city: this.districtsObj.cities[this.city],
						area: this.districtsObj.areas[this.area],
					}
				}
